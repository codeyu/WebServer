...
namespace HttpListener.Test
{
    public class HttpListenerTest
    {
        ...
        private ForkHttpListener.HttpListener StartHttpListener(int port, EventHandler<HttpListenerRequestEventArgs> requestHandler = null)
        {
            address = "127.0.0.1";
            url = (new UriBuilder($"http://{address}:{port}/test")).Uri;

            var listener = new ForkHttpListener.HttpListener(IPAddress.Parse(address), port);
        ...
        public async Task PostRequest()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                Assert.Equal("Hey", responseContent);
            }
